    ...
    try {
        ...
        if((await bcrypt.compare(req.body.password, response.password)) === false) {
            ...
            try{
                ...
                const addError = new Error("Unable to sign-in at this moment. Please try again later!");
...
const removeTempToken = async (Id, token) => {
    try {
        ...
        const removeToken = await prisma.user.update({
            ...
            data: {
                currentToken: user.currentToken.filter((eachToken) => eachToken !== token)
            }
        })
...
const forgotPassword = async (req, res, next) => {
    try {
        ...
        if(error.code === "P2025")
        {
            next({
                statusCode: 200,
                message: "If your email address exists in the system, you will receive an email with instructions to reset your password."
            })
        } else {
            next({
                message: "Unknown Error. Unable to request password reset at this moment. Try again later!"
            })
        }
    }
}
        ...
        if(response){
            if(response.resetPassTokenExpiry < Date.now()){
                ...
                const changePassword = await prisma.user.update({
                    ...
                    data: {
                        password: await bcrypt.hash(newPassword, Math.floor(Math.random() * 10) + 1),
                        resetPassToken: null,
                ...
                res.status(200).json({
                    message: "Password has been succesfully reset. Please login with your new password."
                })
            }
        } else {
            res.status(404).json({
